...
/*   Updated: 2020/03/06 15:14:22 by jchotel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_pipe(int j, int nb_pipes, int *pipes, t_shell *sh)
{
	...
	{
		if (sh->type == 0)
			pipes[j + 1] = open(sh->redir[i], 1537, 0644);
		else
			pipes[j + 1] = open(sh->redir[i], 0521, 0644);
		dup2(pipes[j + 1], 1);
		i++;
	}
...
void	handle_pipe(t_shell *sh)
{
	...
	nb_pipes = ft_arraysize(sh->pipes) * 2 - 2;
	if (!(pipes = (int *)malloc(sizeof(int) * nb_pipes + 1)))
		return ;
